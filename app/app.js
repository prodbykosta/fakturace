...
function invoiceTotal(inv) {
  ...
  for (const l of inv.lines) {
    ...
    subtotal += base;
    vatTotal += base * (Number(l.vatRate) || 0) / 100;
  }
...
/* Další číslo faktury: RRRR-MM-NN v měsíci data vystavení (bez data = dnešek) */
...
document.getElementById("bank-file").addEventListener("change", async e => {
  ...
  saveStore(); renderBank(); renderInvoices(); renderDashboard();
  alert(`Importováno ${newCount} nových transakcí. Automaticky spárováno: ${matchCount}.`);
...
function renderInvoices() {
  ...
  html += `<table class="list-table">
    <thead><tr><th>Číslo</th><th>Klient</th><th>Vystaveno</th><th>Splatnost</th><th class="num">Částka</th><th>Stav</th><th></th></tr></thead>
    <tbody>${sorted.map(inv => {
      ...
      return `<tr class="${inv.status === "cancelled" || inv.status === "deleted" ? "row-cancelled" : ""}">
        <td><strong>${escHtml(inv.number)}</strong>${isProforma(inv) ? ` <span class="badge proforma">Zálohová</span>` : ""}${cur !== 'CZK' ? ` <span class="currency-tag">${cur}</span>` : ""}${inv.imported ? ` <span class="import-mark" title="Importováno AI analýzou">${icon("ai")}</span>` : ""}${inv.recurringId ? ` <span class="import-mark" title="Vystaveno z pravidelné faktury">${icon("repeat")}</span>` : ""}</td>
...
/* doplnit případná nová pole do starších uložených nastavení */
...
function computeTaxEstimate(year) {
  ...
  if (nAct > 0) {
    ...
    if (pocitaneMesice > 0) {
      ...
      const minVZ = nH * Number(ts.socMinVZHlavni || 0) + (vedlejsiPlati ? nV * Number(ts.socMinVZVedlejsi || 0) : 0);
  ...
  if (nAct > 0) {
    ...
    const minVZ = nH * Number(ts.zdravMinVZ || 0);
  ...
  return {
    year, months, nH, nV, nAct, prijmy, vydaje, zaklad, dan, soc, zdrav,
...
document.addEventListener("click", e => {
  ...
  const ts = store.taxSettings;
  for (const k of TAX_FIELDS) taxForm.elements[k].value = ts[k] ?? "";
...
taxForm.addEventListener("submit", e => {
  ...
  if (!ts.months) ts.months = {};
  ts.months[String(taxModalYear)] = [...document.querySelectorAll("#tax-months .tax-month")]
...
function openExpenseModal(data, fromAi) {
  ...
  f.date.value = todayISO();
  if (data) for (const k of ["id", "fileId", "source", "supplier", "amount", "date", "category", "note"]) {
    if (data[k] !== undefined && data[k] !== null) f[k].value = data[k];
  }
...
/* ---- seznam hlášení ---- */
const REPORT_LABELS = { shv: "Souhrnné hlášení", dp3: "Přiznání DPH", kh1: "Kontrolní hlášení", pdf: "PDF dokument" };
  ...
  wrap.innerHTML = `<table class="list-table">
    ...
    <tbody>${sorted.map(r => `
      <tr>
        ...
        <td data-label="Období"><strong>${MONTH_NAMES[r.mesic - 1]} ${r.rok}</strong></td>
...
document.addEventListener("click", e => {
  const b = e.target.closest("button");
  if (!b) return;
  const rid = b.dataset.reportXml || b.dataset.reportEdit || b.dataset.reportDel || b.dataset.reportPdf || b.dataset.reportAttach;
  if (!rid) return;
  ...
  if (b.dataset.reportEdit) r.type === "shv" ? openShvModal(r) : r.type === "kh1" ? openKh1Modal(r) : openDp3Modal(r);
...
function applyRestoredStore(data) {
  store = data;
  for (const k of ["incomes", "expenses", "reports", "recurring", "recurringExpenses", "recurringIncomes", "quotes"]) {
  ...
  renderReports(); renderExpenses(); renderIncomes();
}
